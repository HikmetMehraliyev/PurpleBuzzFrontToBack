...
public class ServicesController : Controller
{
    ...
    [HttpPost]
    public async Task<IActionResult> Update(Service newservice)
    {
        ...
        Service result = await _context.Services.FirstOrDefaultAsync(s => s.Id == newservice.Id);

        if (result is null)
        {
            TempData["Exists"] = "Bu Servis bazada yoxdur";
            return RedirectToAction(nameof(Index));
        }
        //ServiceImage serviceImage = new ServiceImage();
        //if (!CheckPhoto(serviceImage.Path)) ModelState.AddModelError("Photos", _errorMessages);
        //string rootPath = Path.Combine(_enviroment.WebRootPath, "assets", "img");
        //List<ServiceImage> images = await CreateFileAndGetServiceImages(serviceImage.Photos, rootPath);

        result.Name = newservice.Name;
        result.Description = newservice.Description;
        result.Price = newservice.Price;
        result.CategoryId = newservice.CategoryId;
        result.ServiceImages = newservice.ServiceImages;
